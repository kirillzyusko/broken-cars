...
Home and host pages share this clip. Driver pages skip it. Browsers that block audible autoplay get muted playback with a button to restart with sound. Background music waits until the intro ends or the host skips it. Reduced-motion mode keeps the still poster.
