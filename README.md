...
A local, server-authoritative multiplayer party racing prototype. The host opens a waiting room, players scan its QR code and claim a seat, and the host starts a shared 15-second car-building round once everyone has joined. PlayCanvas renders Corsica GP and synchronized modular karts. Cars use responsive arcade acceleration, grip, and steering; holding the brake at a standstill reverses. A round ends as soon as the first kart crosses the line, or after three minutes; everyone still on track is placed by distance. Each car has a 1,000 kg mass; impacts transfer momentum, spin cars on off-center hits, and rebound from barriers. Every submitted car receives three broken parts before the first race: one fatal defect, one critical defect, and one annoying defect. After each ride players get 30 seconds to send one message to the garage. The session ends once every kart is fully repaired.
