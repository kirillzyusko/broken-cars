...
A local, server-authoritative multiplayer party racing prototype. The host opens a waiting room, players scan its QR code and claim a seat, and the host starts a shared 15-second car-building round once everyone has joined. PlayCanvas renders Corsica GP, synchronized modular karts, and three lane barriers. Cars use responsive arcade acceleration, grip, and steering; holding the brake at a standstill reverses. A round ends as soon as the first kart crosses the line, or after three minutes; everyone still on track is placed by distance. Each car has a 1,000 kg mass; impacts transfer momentum, spin cars on off-center hits, and rebound from barriers. Every submitted car receives four compatible broken parts. Assignment normally uses one of three random mixes: one fatal, two critical, and one annoying defect; three critical and one annoying defect; or two critical and two annoying defects. The latter two mixes have no fatal defect, so those cars can move in their first race. Explicit requirements in the player's car prompt take priority: contradicting defects are excluded and replaced with compatible defects from any severity category when necessary. All assigned defects are active and visible together. A repair removes only the current problems the player specifically describes; other defects remain and repaired parts stay fixed. After each ride players get 30 seconds to send one message to the garage. The session ends once every kart is fully repaired.
...
- `src/host/` renders the TV. `HostScreen.jsx` maps room phases to six screens (join, prompt time, grid, race, standings, final) inside `TvStage.jsx`, a fixed 1920×1080 stage scaled uniformly to any display. The race screen is a split screen like a console party racer: every racer gets a third-person chase camera in seat order (side by side for two, a 2×2 grid for three or four, with a spare cell showing the island), and each kart's HUD sits inside its own feed. `src/RaceView.jsx` is a bare-canvas wrapper around `src/race-scene-runtime.js`, which renders every feed through the same post-processing into its own texture and tiles the canvas, so the TV still loads the island once; `src/split-screen.js` owns the tiling shared by the renderer and the HUD grid. `src/RaceScene.jsx` remains the framed panel used by the map preview page. The host's only controls are the yellow buttons that advance the round.
- `src/player/` renders the phone. `PlayerScreen.jsx` maps phases to the claim-a-seat screen (name and colour, stored by the server and locked once the build starts), the waiting room, the one-shot prompt screen, the sent/hold states, the landscape race view with controls over the full-screen track, and the round result.
...
The [kart driving test](http://localhost:3001/map/drive) uses the same free driving code as multiplayer, with world positions and a chase camera that follows the kart's heading. WASD or arrow keys drive; hold S/down to brake, then reverse; hold Space while steering above 6 m/s to drift; R resets the sandbox. Acceleration is quick, steering tightens at low speed, and releasing steering holds the chosen direction. Release Space to regain grip. Drifting and braking leave fading tire marks. There is no jump input. The kart keeps its existing Idle animation, while wheel meshes roll with travel and follow the ground at each tire.
...
The sandbox has no room or lap timer. The collapsible Dev controls panel changes wheels, engine and steering live. Open Driving faults for the remaining defects. Pause driving to inspect parts, restore them in place, or reset to the grid. Click the track or Drive after using the form to resume keyboard controls. It loops the race background music and shares the saved Music on/off setting. Its canvas emits `kart-audio-state` events with speed in metres per second, throttle, brake, steering, and estimated RPM. Driving audio now uses the local kart mixer described below.
...
### Speed and steering requests

With the OpenAI mechanic enabled, build and tuning prompts can request speed and steering changes independently. Ordinary requests use moderate settings; explicit extremes such as “make it ultra fast” can select 8× speed and acceleration. Steering ranges from 0.12× to 4× sensitivity. Unmentioned settings stay unchanged, and new cars default to 1× for both. Ask for normal speed or normal steering to restore that setting. Describing a broken part repairs that part without retuning the kart. The local defect selector does not interpret tuning requests; `/map/drive` has selectors for testing all levels without the mechanic.
