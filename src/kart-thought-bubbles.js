...
function loadArtwork(device, ids, car, entry) {
  ...
  Promise.all(sources.map(loadImage)).then(([cloud, ...icons]) => {
    ...
    const size = icons.length === 1 ? 112 : 64;
    const columns = Math.min(2, icons.length);
    const rows = Math.ceil(icons.length / columns);
    icons.forEach((icon, index) => {
      const x = 96 - columns * size / 2 + (index % columns) * size;
      const y = 102 - rows * size / 2 + Math.floor(index / columns) * size;
      context.drawImage(icon, x * 2, y * 2, size * 2, size * 2);
    });
    const texture = new pc.Texture(device, {
      name: `Kart fault / ${ids.join(",")}`,
      flipY: true,
  ...
  return {
    sync(carStates) {
      ...
      for (const [id, state] of carStates) {
        ...
        const entry = { entity, material, state, signature, ids, artwork: null, size: new Float32Array(2), disposed: false };
        entry.artwork = loadArtwork(app.graphicsDevice, ids, car, entry);
    ...
    update(elapsedMs, reducedMotion) {
      for (const entry of entries.values()) {
        ...
        const page = frame && frame.ids.join(",") === entry.ids.join(",") ? entry.artwork : null;
