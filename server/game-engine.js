...
function advanceCar(player, dt, raceElapsedMs) {
  ...
  const nextLane = clamp(unclampedLane, -1, 1);
...
function applyWorldPosition(car, index, carCount, position) {
  ...
  car.lane = clamp((position.x - grid.x) / LANE_TO_WORLD, -1, 1);
  car.distance = clamp(
    (grid.z - position.z) / DISTANCE_TO_WORLD,
    0,
    TRACK_LENGTH_METERS,
  );
}
...
export class GameEngine {
  ...
  tick(now = Date.now()) {
    ...
    for (const room of this.rooms.values()) {
      ...
      for (const player of racers) advanceCar(player, dt, elapsed);
